...
function download_binary() {
  ...
  womo::log::info "Downloading ${base_url}/${filename}"
  womo::util::download_from_github \
    "${WOMO_TEMP}/${filename}" "${base_url}/${filename}"
}

function setup_binary() {
  womo::log::info "Installing k3d to ${INSTALL_K3D_BIN_DIR}/k3d"
  mkdir -p "${INSTALL_K3D_BIN_DIR}"
  install -m 0755 "${WOMO_TEMP}/k3d-linux-amd64" "${INSTALL_K3D_BIN_DIR}/k3d"
}
