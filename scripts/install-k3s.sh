...
function download_binary() {
  ...
  womo::log::info "Downloading ${base_url}/k3s"
  womo::util::download_from_github "${WOMO_TEMP}/k3s" "${base_url}/k3s"
}

function setup_binary() {
  womo::log::info "Installing k3s to ${INSTALL_K3S_BIN_DIR}/k3s"
  ${SUDO} mkdir -p "${INSTALL_K3S_BIN_DIR}"
  ${SUDO} install -m 0755 "${WOMO_TEMP}/k3s" "${INSTALL_K3S_BIN_DIR}/k3s"

  local command
  for command in kubectl crictl ctr; do
    womo::log::info "Linking ${INSTALL_K3S_BIN_DIR}/${command} to k3s"
